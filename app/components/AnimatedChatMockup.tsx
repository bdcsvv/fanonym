  ...
  return (
    <>
      ...
      <div className={`relative transition-all duration-700 hidden lg:block ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        ...
        {/* Terenkripsi Badge */}
        <div className="absolute bottom-20 -right-4 bg-zinc-800/80 backdrop-blur border border-zinc-700 rounded-xl px-4 py-2 flex items-center gap-2 z-10 animate-fadeIn">
          <svg className="w-4 h-4 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {currentScene === 'creator' && earnedCredits > 0 && (
          <div className="absolute -top-4 left-4 bg-green-600/20 backdrop-blur border border-green-500/30 rounded-xl px-4 py-2 flex items-center gap-2 z-10 animate-bounceIn">
            <span className="text-lg">💰</span>
            <span className="text-sm font-medium text-green-400">+{earnedCredits} Kredit</span>
          </div>
        )}
